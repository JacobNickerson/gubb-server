...
in
{
  ...
  config = lib.mkIf cfg.enable {
    assertions = lib.mapAttrsToList (serviceName: service: {
      assertion =
        !service.cloudflare_tunnel.enable
        || (service.cloudflare_tunnel.useHttpBoilerplate || service.cloudflare_tunnel.default != null);
    ...
    sops = lib.mkMerge [
      {
        secrets."cloudflare/dns_token" = lib.mkIf use_acme { };

        templates."cloudflare.env".content = lib.mkIf use_acme ''
